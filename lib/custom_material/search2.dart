...
/// [SearchDelegate2.close] call. If the search page is closed with the system
/// back button, it returns null.
///
/// A given [SearchDelegate2] can only be associated with one active [showSearch2]
/// call. Call [SearchDelegate2.close] before re-using the same delegate instance
/// for another [showSearch2] call.
///
...
/// The animation provided by [SearchDelegate2.transitionAnimation] can be used
...
/// See also:
///
///  * [SearchDelegate2] to define the content of the search page.
Future<T?> showSearch2<T>({
  required BuildContext context,
  required SearchDelegate2<T> delegate,
  String? query = '',
...
/// Delegate for [showSearch2] to define the content of the search page.
///
/// The search page always shows an [AppBar] at the top where users can
/// enter their search queries. The buttons shown before and after the search
/// query text field can be customized via [SearchDelegate2.buildLeading]
/// and [SearchDelegate2.buildActions]. Additionally, a widget can be placed
/// across the bottom of the [AppBar] via [SearchDelegate2.buildBottom].
///
/// The body below the [AppBar] can either show suggested queries (returned by
/// [SearchDelegate2.buildSuggestions]) or - once the user submits a search  - the
/// results of the search as returned by [SearchDelegate2.buildResults].
///
/// [SearchDelegate2.query] always contains the current query entered by the user
/// and should be used to build the suggestions and results.
///
/// The results can be brought on screen by calling [SearchDelegate2.showResults]
/// and you can go back to showing the suggestions by calling
/// [SearchDelegate2.showSuggestions].
///
/// Once the user has selected a search result, [SearchDelegate2.close] should be
/// called to remove the search page from the top of the navigation stack and
/// to notify the caller of [showSearch2] about the selected search result.
///
/// A given [SearchDelegate2] can only be associated with one active [showSearch2]
/// call. Call [SearchDelegate2.close] before re-using the same delegate instance
/// for another [showSearch2] call.
...
/// ## Handling emojis and other complex characters
/// {@macro flutter.widgets.EditableText.onChanged}
abstract class SearchDelegate2<T> {
  ...
  /// communicate the result back to the initial caller of [showSearch2].
  ...
  /// to [showSearch2] that launched the search initially.
...
enum _SearchBody {
  ...
  ///
  /// The suggested queries are generated by [SearchDelegate2.buildSuggestions].
  suggestions,

  /// Search results are currently shown in the body.
  ///
  /// The search results are generated by [SearchDelegate2.buildResults].
  results,
}
...
class _SearchPageRoute<T> extends PageRoute<T> {
  ...
  final SearchDelegate2<T> delegate;
...
class _SearchPageState<T> extends State<_SearchPage<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    final actions = widget.delegate.buildActions(context) ?? [];
    return Semantics(
      ...
      child: Theme(
        ...
        child: Scaffold(
          appBar: AppBar(
            ...
            automaticallyImplyLeading: widget.delegate.automaticallyImplyLeading ?? false,
            title: Padding(
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Row(
                          mainAxisAlignment: MainAxisAlignment.end,
                          children: [widget.delegate.buildLeading(context)??Text('')] ,
                        ),
                        Expanded(
                          child: Align(
                            ...
                            child: TextField(
                              ...
                              decoration: InputDecoration(hintText: searchFieldLabel,
                                ...
                                suffixIcon: actions.isNotEmpty ? actions[0] : null,
